...
class AddUser extends Model {
    ...
    public function validat($data) {
        ...
        $validate->add('work_sdate', new PresenceOf(
                array(
            'message' => 'working start date is required'
                )
        ));
        $validate->add('dept', new PresenceOf(
                        array(
                    'message' => 'Department is required'
                        )
                ))
                ->add('position', new PresenceOf(
                        array(
                    'message' => 'Position is required'
        )));

        $validate->add('password', new PresenceOf(
                array(
            'message' => "Password is required"
        )));
        $validate->add('confirm', new Confirmation(
                        array(
                    'with' => "password",
