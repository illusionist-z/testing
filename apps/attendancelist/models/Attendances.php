...
class Attendances extends Model {
    ...
    public function searchByTwoOption($search_date_month,$search_date_year,$search_dept,$currentPage)  {
        try {
            $currentmth = date('m');
            $currentYr = date("Y");
            if ($search_date_month == '-Select Month-'){
                $this->session->search;
            }
          ...
          if ($search_dept == 'Select Dept'){
            ...
            else if($search_date_month == '-Select Month-'){
                $row = $this->modelsManager->createBuilder()
                    ->columns(array("core.member_login_name", "group_concat(DAY(attendances.att_date)) as day"
                        . ",attendances.member_id,group_concat(attendances.status) as status"))
                    ->from(array('core' => 'salts\Core\Models\Db\CoreMember'))
            ...
            else if($search_date_year == 'Year' && $search_date_month == '-Select Month-'){
                 $row = $this->modelsManager->createBuilder()
                    ->columns(array("core.member_login_name", "group_concat(DAY(attendances.att_date)) as day"
                        . ",attendances.member_id,group_concat(attendances.status) as status"))
                    ->from(array('core' => 'salts\Core\Models\Db\CoreMember'))
             ...
             else if($search_date_year == 'Year' && $search_date_month == '-Select Month-' && $search_dept == 'Select Dept'){
                 $row = $this->modelsManager->createBuilder()
                    ->columns(array("core.member_login_name", "group_concat(DAY(attendances.att_date)) as day"
                        . ",attendances.member_id,group_concat(attendances.status) as status"))
                    ->from(array('core' => 'salts\Core\Models\Db\CoreMember'))
             ...
             else if($search_date_year == 'Year'  && $search_dept == 'Select Dept'){
                 $row = $this->modelsManager->createBuilder()
                    ->columns(array("core.member_login_name", "group_concat(DAY(attendances.att_date)) as day"
                        . ",attendances.member_id,group_concat(attendances.status) as status"))
                    ->from(array('core' => 'salts\Core\Models\Db\CoreMember'))
